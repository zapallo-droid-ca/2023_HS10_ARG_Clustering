...
tab0 = tab1.merge(df_base[['partnerCodeISO','METRICS GROUP','SIMILARITY GROUP','TREND GROUP','SEASONAL GROUP', 'RESIDUAL GROUP']].drop_duplicates(), on = 'partnerCodeISO',how = 'left')
...
def valOfArgExportsInMUSD():
    ...
    for year in years:        
        metric3 = np.round( dfY[(dfY['reporterISO'] == 'ARG') & (dfY['flow'] == 'Export') & (dfY['year'] == year)].totalAnnualValue.sum() / 1000000,1)
        metric4 = np.round( dfY[(dfY['reporterISO'] == 'ARG') & (dfY['flow'] == 'Export') & (dfY['year'] == year)].totalCereals.sum() / 1000000,1)
        metric5 = np.round( dfY[(dfY['reporterISO'] == 'ARG') & (dfY['flow'] == 'Export') & (dfY['year'] == year)].maize.sum() / 1000000,1)        
        
        df_temp = df_temp.append({'Year':year,'Total Exports':metric3,'Cereals Exports':metric4,'Maize Exports':metric5}, ignore_index = True)
    
    df_temp = df_temp.sort_values('Year', ascending = True).reset_index(drop = True)
    df_temp['Year'] = df_temp['Year'].astype('str').str[:4]
...
def AnnualGainInExports():
    ...
    tab6 = tab2.copy()[['year','totalCereals']]
    tab6 = tab6.merge(tab6.set_index('year').diff().reset_index(), on = 'year', how = 'left').fillna(0).iloc[1:]
    tab6['%exportsGain'] = np.round( (tab6['totalCereals_y'] / tab6['totalCereals_x']) * 100, 1)
    tab6.rename(columns = {'year':'Year','%exportsGain':'Gain %'}, inplace = True)
    tab6 = tab6[['Year','Gain %']]
    tab6['Time Series'] = 'Total Cereals'
    
    tab7 = tab3.append(tab4)
    tab7 = tab7.append(tab5)
    tab7 = tab7.append(tab6)
    tab7 = tab7.reset_index(drop = True).sort_values('Year', ascending = True)
    tab7['Year'] = tab7['Year'].astype(str).str[:4]
    
    titleText = '% Gain of Annual Values'

    fig = px.line(tab7, x = 'Year', y ='Gain %',color = 'Time Series' ,title = titleText, width=900, height=450)
    return fig
...
def percentageOfChangeCerealsExp():
    ...
    df_temp['% of Change 2022-2021'] = np.round(((df_temp['Value_2022'] - df_temp['Value_2021']) / df_temp['Value_2021']) * 100 ,1) 
...
def metricsClusterCentroids():    
    df_plot = df_centersExp01.drop(columns = 'cereal').copy()
    clusterTag = len(df_plot.cluster)
    ...
    plt.title('Var Importance in Clusters')
    
    #kmeansCentersDeScaling = pd.read_csv(wd + '/clustersExp01_centersDs.csv.gz', sep = csvAttr_imp['sep'], encoding = csvAttr_imp['encoding'])
           
    return plt.show() #display(kmeansCentersDeScaling)


def metricsClusterCentroidsPx():
